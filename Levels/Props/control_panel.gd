...
const DragNDropMiniGame = preload("res://Levels/Mini-games/DragNDropMiniGame.tscn")
...
func _on_interact(state : bool, objectId : int):
	if state == true and objectId == self.objectID and self.solved == false:
		...
		var DragNDropGameInstance = DragNDropMiniGame.instantiate()
		self.add_child(DragNDropGameInstance)
