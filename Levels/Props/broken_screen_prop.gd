...
const screenMiniGame = preload("res://Levels/Mini-games/screenMiniGame.tscn")
...
func _physics_process(delta: float) -> void:
	...
	if solved == true:
		self.sprite.texture = load("res://Assets/Props/screenFixed.png")
...
func _on_interact(state : bool, objectId : int):
	if state == true and objectId == self.objectID and self.solved == false:
		...
		var screenGameInstance = screenMiniGame.instantiate()
		self.add_child(screenGameInstance)
